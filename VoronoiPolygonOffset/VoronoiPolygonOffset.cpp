	...
	for (auto& offset : offsets)
	{
		if (offset.get_ID() >= IDOfInitialOffset)
		{
			for (int j = 0; j < offset.get_edges().size(); j++)
			{
				...
				set<VertexGenerator2D*> vtxGeneratorsOfSV;
				set<VertexGenerator2D*> vtxGeneratorsOfEV;
				
				if (generatorOfStartVtx.first->getType() == Generator2D::VERTEX_G)
					vtxGeneratorsOfSV.insert(static_cast<VertexGenerator2D*>(generatorOfStartVtx.first));
				if (generatorOfStartVtx.second->getType() == Generator2D::VERTEX_G)
					vtxGeneratorsOfSV.insert(static_cast<VertexGenerator2D*>(generatorOfStartVtx.second));

				if (generatorOfEndVtx.first->getType() == Generator2D::VERTEX_G)
					vtxGeneratorsOfEV.insert(static_cast<VertexGenerator2D*>(generatorOfEndVtx.first));
				if (generatorOfEndVtx.second->getType() == Generator2D::VERTEX_G)
					vtxGeneratorsOfEV.insert(static_cast<VertexGenerator2D*>(generatorOfEndVtx.second));

				VertexGenerator2D* commonVtxGenerator = nullptr;
				for (auto& generatorOfSV : vtxGeneratorsOfSV)
				{
					for (auto& generatorOfEV : vtxGeneratorsOfEV)
					{
						if (generatorOfSV == generatorOfEV)
							commonVtxGenerator = generatorOfSV;
					}
				}


				if (commonVtxGenerator != nullptr)
				{
					currEdge->set_is_arc_edge(true);
					currEdge->set_reflex_vertex(commonVtxGenerator);
